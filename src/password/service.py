...
@dataclass
class PasswordService(PasswordRepository):
    """Сервис для кодов сброса пароля"""
    ...
    async def send_code_for_change_password(self, user_schemas: UserBaseSchemas) -> dict:
        ...
        user = await self.get_user_by_email(email=user_schemas.email)
        if not user:
            raise UserDontExist
        code = uuid.uuid4()
        ...
        await self.vault_service.create_secret(user_id=user.id,
        ...
        await self.create_object(save_object=code_in_db)
    ...
    async def change_password(self, user_schemas: UserChangePasswordSchemas) -> dict:
        ...
        code = await self.find_code(code=user_schemas.code)
        if not code:
            raise BadCode
        user = await self.find_user_by_code(code_email=code.user_email)
        if not user:
            raise UserDontExist
        ...
        await self.vault_service.create_secret(user_id=user.id,
                                               password=hash_password(password=new_password))
        await self.delete_object(delete_object=code)
